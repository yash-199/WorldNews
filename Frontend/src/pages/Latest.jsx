...
const Latest = () => {
    ...
    useEffect(() => {
        const fetchDate = async () => {
            try {
                const response = await fetch('https://newsapi.org/v2/everything?q=news&sortBy=publishedAt&apiKey=321e29923f4d483ebca260b3fe5304a8&language=en');
                const jsonData = await response.json();
                setLatestData(jsonData.articles || []);
            } catch (error) {
    ...
    return (
        <div className='w-full mt-16 px-4 md:px-6 lg:px-10 xl:px-16'>
            <h1 className='text-center text-3xl md:text-4xl font-semibold text-black mb-8'>Latest News</h1>
                    ...
                    >
                        <div className='relative mb-6'>
                            <img
                                ...
                                alt="Main topic"
                            />
                            <p className='absolute right-4 transform -translate-y-1/2 text-white text-sm bg-red-700 px-3 py-1 rounded' style={{ top: "12%" }}>Hot Topic</p>
